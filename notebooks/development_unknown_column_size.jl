...
md"""
# Development of the simulation of GC systems for unknown column lengths or column diameters
"""
...
# ╔═╡ aeacbcbf-3f86-4429-b219-6858cf5b9918
ModuleColumn_("test", NaN, 0.25e-3, 0.25e-6, "DB5", default_TP)

# ╔═╡ 50d7ae2d-c3bc-4262-89c3-a9251bcbd9bc
ModuleColumn_("test", 30.0, NaN, 0.25e-6, "DB5", default_TP)
...
begin
	...
	pp_split[1] = GasChromatographySystems.PressurePoint("p₁", [0.0, 1800.0], [204000.0, 267000.0]) # inlet 
	...
	modules_split[2] = ModuleColumn_("TL column 1", NaN, 0.1e-3, 0.0e-6, "", 300.0, 0.5/60e6)
	modules_split[3] = ModuleColumn_("TL column 2", NaN, 0.15e-3, 0.15e-6, "SLB5ms", 300.0, 0.5/60e6)
	# system
...
# ╔═╡ 71009f3b-d39a-4c7e-9495-221868d2c898
function unknown_L(sys)
	i_unknown_lengths = Int[]
	for i=1:ne(sys.g)
		if isnan(sys.modules[i].length)
			push!(i_unknown_lengths, i)
		end
	end
	return i_unknown_lengths
end

# ╔═╡ 32e653ad-46ff-407e-8b85-01f7ee8f8410
function unknown_d(sys)
	i_unknown_diameters = Int[]
	for i=1:ne(sys.g)
		if isnan(sys.modules[i].diameter)
			push!(i_unknown_diameters, i)
		end
	end
	return i_unknown_diameters
end

# ╔═╡ 902b561a-0014-4628-9b57-66b87b6796ef
sys_split.modules[1].length
...
# ╔═╡ 5b3b7b16-7087-48f0-bba8-3864f7ac571e
unknown_L(sys_split)

# ╔═╡ dc79f696-9d21-4da4-a538-8d39fc4ea3c5
unknown_d(sys_split)
...
# ╔═╡ 2b849d07-4e96-4921-847c-ea0faf0709f4
unknown_L(sys_split)

# ╔═╡ 1f914b32-a8a3-4360-b5ff-9f2126eec947
unknown_d(sys_split)

# ╔═╡ 937ca41d-b6ec-4e6a-b27f-f3c2bc552d50
eq_split = substitute_unknown_flows(sys_split)

# ╔═╡ 96dd9da2-74ba-46b1-a3ab-64bf56735613
i_unknown_p = GasChromatographySystems.unknown_p(sys_split)

# ╔═╡ dfdb1f7e-c73f-4b1f-b965-56dab005ec62
i_unknown_F = unknown_F(sys_split)

# ╔═╡ 84620a18-43fc-4948-9576-238029ee69df
i_unknown_Ld = union(unknown_L(sys_split), unknown_d(sys_split))

# ╔═╡ 85d74c88-1200-45db-88f7-6cea93428c05
(length(i_unknown_p) + length(i_unknown_Ld)) == length(eq_split)

# ╔═╡ 74148ea4-d8cb-46d3-9258-c09c89d8a0cc
sys_split

# ╔═╡ 08699aaa-9ac8-46c7-a882-44000c3a0993
#@variables κ⁻¹

# ╔═╡ e8bb0a87-45e0-44a5-bb01-29d054df80bd
Symbolics.solve_for(eq_split, [P²[1], P²[2], κ⁻¹[2], κ⁻¹[3]])

# ╔═╡ 36f15aa8-e98b-4fe5-8d12-91f70d3f94de
Symbolics.solve_for(eq_split[2:4], [P²[2], κ⁻¹[2], κ⁻¹[3]])
...
function solve_balance(sys)
	...
	i_unknown_L = unknown_L(sys) # indices of the edges with an unknown column length
	i_unknown_d = unknown_d(sys) # indices of the edges with an unknown column diameter
	i_unknown_Ld = union(i_unknown_L, i_unknown_d)
	bal_eq = substitute_unknown_flows(sys)
	#num_use_eq = GasChromatographySystems.unknowns_in_flow_balances(sys)[2]
	if (length(i_unknown_p) + length(i_unknown_Ld)) == length(bal_eq)
