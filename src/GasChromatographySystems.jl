...
function simulate_along_paths(sys, paths, db_dataframe, selected_solutes; t₀=zeros(length(selected_solutes)), τ₀=zeros(length(selected_solutes)))
	...
	path_pos, peaklists, solutions, new_par_sys = simulate_along_paths(sys, paths, db_dataframe, selected_solutes, par_sys; t₀=t₀, τ₀=τ₀)
	return path_pos, peaklists, solutions, new_par_sys
end

function simulate_along_paths(sys, paths, db_dataframe, selected_solutes, par_sys; t₀=zeros(length(selected_solutes)), τ₀=zeros(length(selected_solutes)), refocus=falses(ne(sys.g)), τ₀_focus=zeros(length(selected_solutes)))
	#par_sys = graph_to_parameters(sys, db_dataframe, selected_solutes)
	E = collect(edges(sys.g))
