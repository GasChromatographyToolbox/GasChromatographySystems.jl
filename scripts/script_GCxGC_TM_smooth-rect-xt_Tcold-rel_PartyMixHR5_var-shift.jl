...
#=
- smooth rectangle temperature function over time and space at modulation point (`spatial=false`, `tflank=20`, `algTM=Vern9` and `ng=true`)
- Tcold is relative (`Tcold_abs=false`)
- variation of the modulator shift and its influence on the retention times
=#

optTM = GasChromatographySystems.ModuleTMOptions(Tcold_abs=false, ng=false, alg=Vern9(), tflank=20, sflank=40, dtinit=0.5e-7)
...
## Save Result
CSV.write(joinpath(pwd(), "scripts", "var-shift_rect-xt_PartyMixHR5.csv"), res)

settings = Dict(fieldnames(GasChromatographySystems.System) .=> getfield.(Ref(sys[1]), fieldnames(GasChromatographySystems.System)))
CSV.write(joinpath(pwd(), "scripts", "var-shift_rect-xt_PartyMixHR5_settings.csv"), settings)