...
optTM = GasChromatographySystems.ModuleTMOptions(Tcold_abs=false, ng=true, alg=Vern9(), tflank=20, sflank=Inf, dtinit=0.5e-7)
...
## Save Result
CSV.write(joinpath(pwd(), "scripts", "var-shift_PartyMixHR5.csv"), res)

settings = Dict(fieldnames(GasChromatographySystems.System) .=> getfield.(Ref(sys[1]), fieldnames(GasChromatographySystems.System)))
CSV.write(joinpath(pwd(), "scripts", "var-shift_PartyMixHR5_settings.csv"), settings)