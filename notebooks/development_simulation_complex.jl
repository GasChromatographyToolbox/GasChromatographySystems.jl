...
begin
	# setup of the Graph
	g_multisplit = SimpleDiGraph(9)
	add_edge!(g_multisplit, 1, 2) # Inj->GC1->pressure regulator 
	add_edge!(g_multisplit, 2, 3) # pressure regulator -> GC2 -> split
	add_edge!(g_multisplit, 3, 4) # Split -> TL1 -> Det1
	add_edge!(g_multisplit, 3, 5) # Split -> TL2 -> Modulator inlet
	add_edge!(g_multisplit, 5, 6) # Modulator inlet -> Modulator -> GC3 inlet
	add_edge!(g_multisplit, 6, 7) # GC3 inlet -> GC3 -> Split
	add_edge!(g_multisplit, 7, 8) # Split -> TL3 -> Det2
	add_edge!(g_multisplit, 7, 9) # Split -> TL4 -> Det3
	#edge_labels = ["GC₁", "GC₂", "TL₁", "TL₂", "Modulator", "GC₃", "TL₃", "TL₄"]
	#node_labels = ["p₁", "p₂", "p₃", "p₄", "p₅", "p₆", "p₇", "p₈", "p₉"]
	# pressure points
	pp_multisplit = Array{GasChromatographySystems.PressurePoint}(undef, nv(g_multisplit))
	pp_multisplit[1] = GasChromatographySystems.PressurePoint("p₁", [0.0, 1800.0], [200000.0, 200000.0]) # inlet 
	pp_multisplit[2] = GasChromatographySystems.PressurePoint("p₂", [0.0, 1800.0], [NaN, NaN]) # 
	pp_multisplit[3] = GasChromatographySystems.PressurePoint("p₃", [0.0, 1800.0], [NaN, NaN]) #  
	pp_multisplit[4] = GasChromatographySystems.PressurePoint("p₄", [0.0, 1800.0], [101300.0, 101300.0]) # outlet 
	pp_multisplit[5] = GasChromatographySystems.PressurePoint("p₅", [0.0, 1800.0], [NaN, NaN]) #
	pp_multisplit[6] = GasChromatographySystems.PressurePoint("p₆", [0.0, 1800.0], [NaN, NaN]) # 
	pp_multisplit[7] = GasChromatographySystems.PressurePoint("p₇", [0.0, 1800.0], [NaN, NaN]) #
	pp_multisplit[8] = GasChromatographySystems.PressurePoint("p₈", [0.0, 1800.0], [eps(Float64), eps(Float64)])#[0.0, 0.0]) # outlet 2
	pp_multisplit[9] = GasChromatographySystems.PressurePoint("p₉", [0.0, 1800.0], [101300.0, 101300.0]) # outlet 3
	# modules
	modules_multisplit = Array{GasChromatographySystems.AbstractModule}(undef, ne(g_multisplit))
	modules_multisplit[1] = GasChromatographySystems.ModuleColumn("GC column 1", 30.0, 0.25e-3, 0.25e-6, "SLB5ms", default_TP)
	modules_multisplit[2] = GasChromatographySystems.ModuleColumn("GC column 2", 30.0, 0.25e-3, 0.25e-6, "SPB50", default_TP)
	modules_multisplit[3] = GasChromatographySystems.ModuleColumn("TL column 1", 3.0, 0.25e-3, 0.25e-6, "SPB50", 300.0)
	modules_multisplit[4] = GasChromatographySystems.ModuleColumn("TL column 2", 1.0, 0.25e-3, 0.25e-6, "SPB50", 300.0)
	modules_multisplit[5] = GasChromatographySystems.ModuleColumn("Modulator", 0.3, 0.1e-3, 0.1e-6, "Wax", 300.0) # replace later with ModuleModulator
	modules_multisplit[6] = GasChromatographySystems.ModuleColumn("GC column 3", 4.0, 0.1e-3, 0.1e-6, "Wax", default_TP)
	modules_multisplit[7] = GasChromatographySystems.ModuleColumn("TL column 3", 2.5, 0.1e-3, 0.1e-6, "", 300.0)
	modules_multisplit[8] = GasChromatographySystems.ModuleColumn("TL column 4", 1.5, 0.1e-3, 0.1e-6, "", 300.0)
	# system
	sys_multisplit = GasChromatographySystems.update_system(GasChromatographySystems.System(g_multisplit, pp_multisplit, modules_multisplit, GasChromatographySystems.Options(ng=true)))
end

# ╔═╡ ea5d611b-6b21-4185-94fc-ce3a6293c0dd
GasChromatographySystems.plot_graph(sys_multisplit; color=:orange)
...
GasChromatographySystems.flow_balance(sys_multisplit)

# ╔═╡ 40c2806b-ed48-4948-8e8c-2e7c635b76f9
GasChromatographySystems.solve_balance(sys_multisplit)

# ╔═╡ 73f9a1c4-73c9-4690-a1ad-fcadeb6d575a
p_func = GasChromatographySystems.pressure_functions(sys_multisplit)

# ╔═╡ 46bec177-cdf1-4527-9862-daafe5af6828
F_func = GasChromatographySystems.flow_functions(sys_multisplit)

# ╔═╡ 06d0e7d5-4370-4901-88a5-728001c2bee3
GasChromatographySystems.plot_graph_with_flow(sys_multisplit, 0)

# ╔═╡ 0ac6c03b-5be1-41ea-ad99-380a670eee62
GasChromatographySystems.plot_graph_with_flow(sys_multisplit, 1800)

# ╔═╡ 7783216f-6a73-44aa-98f7-e0ca628ba145
begin
	plotly()
	com_timesteps = GasChromatographySystems.common_timesteps(sys_multisplit)
	trange = 0:sum(com_timesteps)/100:sum(com_timesteps)
	p_flow = Plots.plot(xlabel="time in s", ylabel="flow in mL/min")
	for i=1:ne(sys_multisplit.g)
		Plots.plot!(p_flow, trange, F_func[i].(trange)*60*1e6, label="F_$(sys_multisplit.modules[i].name)")
	end
	p_flow
end
...
unique(GasChromatographySystems.all_stationary_phases(sys_multisplit))

# ╔═╡ 2eff2573-137d-4a85-9bc9-e43d081b8d77
GasChromatographySystems.common_solutes(db, sys_multisplit)
...
par_multisplit = GasChromatographySystems.graph_to_parameters(sys_multisplit, db, selected_solutes)

# ╔═╡ 6dce6315-333a-4633-9c35-77a45e041b06
par_multisplit[1].prog.pout_itp(0)

# ╔═╡ bff96e1c-fb9f-48b9-8ea7-2b9295054b8d
par_multisplit[1].prog.pout_itp(1800)
...
paths = GasChromatographySystems.all_paths(sys_multisplit.g, 3)[2]
...
simulate_along_paths_brute_force(sys_multisplit, paths, db, selected_solutes)
...
simulate_along_paths(sys_multisplit, paths, db, selected_solutes)
