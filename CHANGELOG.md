...
- Tests: `Valve junction slicing` (area conservation, slice ordering, `valve_initial_width` / `select_valve_initial_width`, sharp-band `change_initial`); `PeriodicValveProgram pressure interpolation grid` (flat `p₂` per valve phase, no ramp at `t=2 s`); `GCxGC_DPM hydraulics` (`solve_balance`, closed/open `p₂` vs series tee and `p₄`, `holdup_time_functions`, `graph_to_parameters`, `path_possible`); `GCxGC_DPM end-to-end path simulation` (`simulate_along_paths`, multi-period `v*` slices on col 2); `change_initial finite-row guard`.
...
### Changed
- `interpolate_pressure_functions` on tee graphs with `PeriodicValveProgram`: piecewise-constant `p₂(t)` on valve phase segments (`periodic_valve_pressure_time_steps`), avoiding linear ramps across open/closed switches when `interp=true`; ForwardDiff-safe evaluation for column ODEs.
