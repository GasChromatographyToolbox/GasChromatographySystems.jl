...
#@testset "example systems" begin
    ...
    @test ex_GCxGC_TM.options.mobile_phase == "He"
    ...
    @test par_series[1].col.sp == ex_series.modules[1].stationary_phase
    par_split = GasChromatographySystems.graph_to_parameters(ex_split, db_dataframe, selected_solutes)
    @test par_split[2].col.sp == ex_split.modules[2].stationary_phase  
    ...
    @test par_GCxGC_TM[4].col.sp == ex_GCxGC_TM.modules[4].stationary_phase 
#end
